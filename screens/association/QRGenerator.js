...
const QRGenerator = () => {
  ...
  const [item, setItem] = useState("Voluntree:" + actividad.titulo);
  ...
  return (
    <SafeAreaView className="flex w-full h-full bg-[#e6e6e6] items-center space-y-12 justify-start">
      <View className="flex-row flex-wrap w-full h-auto items-center space-x-2 p-1 bg-bottomTabs">
        <TouchableOpacity onPress={navigation.goBack} className="flex h-14 w-14 items-center justify-center">
          <Icon className="h-full w-full items-center justify-center" name="arrow-left" type="octicon" color="black" size={28}/>
        </TouchableOpacity>
