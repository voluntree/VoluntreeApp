...
test("compara fecha mas", async () => {
  const act1 = await getActivityById("Plantación en la Vallesa");
...
test("seguir a una asociacion", async () => {
  const user = {
    actividades: ["Plantación en la Vallesa"],
  ...
  await followAsociation(user, "Modepran")
  await sleep(1000)
  const asociacion = await getAsociationByID("Modepran");
  expect(asociacion.seguidores.includes(user.nombre + " " + user.apellidos)).toBe(true)
});
